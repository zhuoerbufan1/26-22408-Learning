...
一个 dist数组保存源点到其余各个节点的距离，`dist[i] ` 表示源点到节点 `i ` 的距离。初始时，` dist ` 数组的各个元素为无穷大，` dist[1] = 0 `
...
`d[k][i][j]` 表示顶点 i  经过顶点编号不超过 k 的顶点的中转到达和顶点 j 的最短路径长度

`d[0][i][j]` 表示顶点 i 直接到达顶点 j 的最短路径长度，`d[0][i][j]` 在初始化图的时候就可以得到，如果 i 不能直接到达 j，则设置为无穷

对于 `d[k][i][j]`，即顶点 i 经过顶点编号不超过 k 的顶点的中转到达顶点 j，其路径可以分为两种：

（1）路径中含有顶点 k，此时这个路径可以分为两端，从 i -> k 以及从 k -> j，从 i -> k 的路径中没有顶点 k，即经过的是前 k - 1 个顶点的中转，最短是 `d[k - 1][i][k]`，同理，从 k -> j 的路径长度就是 `d[k - 1][k][j]`，这种情况的路径的最短长度是 `d[k - 1][i][k] + d[k - 1][k][j]`

（2）路径中不含有顶点 k，那这种路径就是经过了前 k  - 1 个顶点的中转，所以最短长度就是 `d[k - 1][i][j]`

所以经过顶点 k 的路径的最短长度就是上面两种路径的长度取小：

$$
d[k][i][j] = min \left( d[k - 1][i][j], d[k - 1][i][k] + d[k - 1][k][j] \right)
$$

这个就是 Floyd 算法的递推公式，我们初始的时候可以知道 d[0][i][j]的各种值，所以三重循环就可以得到 ` d[n][i][j]` 了

但是最外层的 k 其实可以提出来当作最外层循环，这样可以将空间降到 n^2，用 `d[i][j]` 来表示从 i 到 j 的最短距离

因为 k 是从小到达循环的，所以在更新 `d[i][j]` 的时候 `d[i][j]` 原来的值其实就是 `d[k - 1][i][j]`，核心代码就是比较 `d[i][j]` 与 `d[i][k] + d[k][j]` 的大小即可
...
即边表示活动的网，`AOE` 网络是一个带权的有向无环图，其中顶点表示事件，弧表示活动（或者工序），弧上的权值表示活动持续的时间，比如下图的一个 `AOE` 网络：
...
活动的最早开始时间就是活动弧起点事件的最早开始时间，如上图所示活动 `a8` 的最早开始时间就是 `10`

## 活动最晚开始时间：

活动的最晚开始时间就是活动箭头起点事件的最晚开始时间，如上图所示活动 `a9` 的最晚开始时间就是 `10`
