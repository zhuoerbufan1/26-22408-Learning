...
（1）Prim 算法在加入一个顶点 V到集合 S 之后需要更新 V 的所有邻接点到集合 S 的距离，这个操作是 V，所以总的复杂度是 $O(V^{2})$（邻接矩阵存储）
...
### 多源点多汇点的AOE网络关键路径求解（求解AOE 网络关键路径的加强版算法，了解即可）
...
（3）每个元素查找失败的比较次数是从根结点到这个失败结点路径经过的结点数（包括根结点但是**不包括失败结点**）

### 分块查找

顺序查找的平均查找长度上文所述，这里不多说
...
得到：
$$
h - 1 \geq \log_{k}r或者h-1 = \log_{k}r
$$
