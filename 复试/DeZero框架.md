...
正向传播非常简单，输入变量 > 0 的话就是输入变量本身，< 0 的话就是 0

反向传播也是根据输入来的，输入 > 0 就将输出变量的导数原样返回，< 0 则导数就是 0
...
（1）lr，用于更新参数的时候的步长，下图公式中的 $\eta$
（2）momentum，用于更新梯度积累的时候的参数，下图公式中的 $\gamma$
（3）vs，一个字典，用于存放在更新过程中不同参数的梯度积累

**基础方法**

（1）update_one，按照下面的公式进行更新就是了，基本过程是，从字典中取出当前参数的梯度积累，没有积累的化初始化为 0，然后乘一个参数 $\gamma$更新梯度积累，然后利用步长 lr 来更新参数
...
（1）保存将要广播的形状 shape
（2）保存数组原本的形状_shape
（3）forwad 函数是调用 broadcast_to 函数广播成 shape 形状
（4）backward 方法是调用 sum_to 形状，传入的参数是_shape，恢复到原来的形状并求和（沿着原来广播的方向进行求和）

求和：
（1）保存将要求和的形状，shape
（2）保存数组原本的形状_shape
（3）forwad 函数是调用 utils. sum_to 函数，将 `_shpae` 与 shape 进行对比，求和成 shape 的形状

#### sum_to 函数的功能

![image.png](https://typora-1310242472.cos.ap-nanjing.myqcloud.com/typora_img/20260317173835.png)

举一个例子，假如 x 的维度是 `(2, 3, 4)`，这里的 shpae = `(1, 4)`，sum_to 函数的功能就是对 x 沿着轴 0 和轴 1 进行求和并保留轴 1 的维度，其实如果 shape 与 x.shape 沿着右对齐的话可以看成 `(2, 3, 4) -> ( , 1, 4)`，即沿着轴 0 求和不保留维度再沿着轴 1 求和保留维度

首先 ndim = 2 是要转换的维度的个数，这里是 2

lead 是原本 x 的维度个数（3）和 shape 维度个数（2）的差值，它的作用是还原 shape 中的维度数为 1 的轴下标进行右对齐之后的轴下标，比如这里 shape  `(1, 4)`，这里维度为 1 的轴下标是 axis = 0，但是右对齐之后 `( , 1, 4)`，维度 1它的轴下标应该是 axis = 1，所以就得将加上 lead，即对齐之后的空缺数才行


`lead_axis` 是空缺的地方所对应的轴，比如这里它的值就是 `(0, )`，这个是需要求和的第 0 轴，它直接按照空缺数 lead 顺序生成即可

`for i, sx in enumerate(shape)` 是取出 shape 中所有的下标和下标所对应的值，比如这里取出的结果是 (0, 1)，(1, 4)，然后加上 if 是只取出维度为 1 的下标和下标对应维度值组合

因此 `for i, sx in enumerate(shape) if sx == 1` 的效果就是取出了 (0, 1)，它的含义就是 `shape = (1, 4)` 的第 0 轴，以及第 0 轴的维度 1

然后 `i + lead` 的效果就是只取出(0, 1) 的下标 0，并加上 lead，也就是加上右对齐之后的空缺将原来 shape 中的轴下标还原成右对齐之后的轴下标

所以这里的 `axis` 变量就是 (1, )，它存放的是 shape = (1, 4) 中所有维度为 1 的轴下标，还原成右对齐之后的轴下标

然后这里的 lead_axis + axis，这是拼接的意思，结果是 (0, 1)，这样就沿着轴 0 和轴 1 进行 sum 求和了，并保留了维度，所以此时得到的 y 的形状是 `(1, 1, 4)`

然后调用 squeeze 函数，这个函数的作用是移出数组中所有维度为 1 的轴，由于 y 的形状是 `(1, 1, 4)` 而目的 shape 的形状是 `(1, 4)`，所以实际上就是移除所有 shape = (1, 4) 右对齐之后空缺地方的轴，也就是 lead_axis 中的轴了
...
（1）前馈神经网络是无环的，信息只在输入层到输出层单向传播
（2）循环神经网络具有环状结构，节点间存在反馈连接，可以处理时间序列数据或者序列依赖性问题

### 优化器

#### 选择优化器中看中哪些参数

（1）学习率：也就是更新参数的时候步长，如果太高，则会来回跳动难以收敛，如果太小则会收敛非常缓慢
（2）动量：可以帮助优化器在梯度下降过程中加速收敛或者跳出局部最小值
（3）批量大小：每次更新模型参数的时候使用多少样本数据，小批量可以使得优化器更具噪声，有助于跳出局部最优，但是可能不稳定；大批量数据可以稳定到达全局最优，但是需要大量计算资源
（4）权重衰减：也就是 L 1 或者 L 2 正则化
（5）初始参数：也就是参数初始化的时候需要注意的事项
（6）自适应性：Adam 优化器可以根据梯度的历史信息自适应调整学习率
#### 牛顿法
...
（1）简单快速，不需要复杂的指数和分段计算
（2）ReLU 可以让很多单元的输出为 0，减少过拟合发生
（3）它可以缓解梯度消失问题，因为在正区间它的梯度恒为 1，不会受到输出输出参数初始化影响

**缺点**

（1）某些神经元如果进入到了 RuLU 函数的负区间，则再也激活不了，输出为 0，梯度也无法传递，无法参与模型学习
（3）这个函数是非对称的

**优化**

Leaky ReLU 函数在负区间引入了一个很小的梯度，让梯度可以继续传播
...
通过在损失函数中假如权重的 L 1 或者 L 2 范数变为惩罚项来限制模型的复杂度，防止权重过大，导致过拟合
...
这种方法是一种在学习的过程中随机删除神经元的方法，训练的时候随机选出隐藏层的神经元，然后将其删除（输出设置为 0），通过这种方法来降低模型复杂度

它的反向传播类似 ReLU 函数，没有被屏蔽的参数相当于做了一个等价变换，那么导数也是原样返回即可，被屏蔽的参数则传播的导数是 0

集成学习：用多个结构类似的网络分别进行学习训练，测试的时候取多个网络结果的平均值

Dropout 有点像利用单个网络进行多次学习，最后输出结果按照在训练的时候被丢弃的概率进行相乘得到平均值

## 卷积神经网络

### CNN 的主要组成

（1）卷积层，负责提取输入数据的特征，通过卷积核扫描输入图片，来提取局部特征
（2）激活函数，通常使用 ReLU 函数，引入非线性
（3）池化层，对邻近区域进行采样，减少数据数量和参数数目，降低复杂度，减少过拟合；常见的池化操作有最大池化和平均池化
（4）全连接层，将之前层收集的特征进行整合，通过 Softmax 函数完成分类或者回归任务

### 卷积层的基本参数和关系

（1）卷积核的通道数与输入特征图的通道数一致
（2）卷积核的个数与输出特征图的通道数一致

（1）卷积核
（2）步幅
（3）填充

输出特征图的高 = 输入特征图的高 + 2 倍的填充圈数 - 卷积核的高再除以步幅，然后 + 1
![image.png|299](https://typora-1310242472.cos.ap-nanjing.myqcloud.com/typora_img/20260309101319.png)

### 卷积层和全连接层的区别

全连接层中每个神经元都与上一层的所有神经元相连接，一般用于最终的的基层

卷积层每个神经元只与局部的一部分精神元相连接，这部分成为感受野，卷积层通过卷积核来提取图像的局部特征

### 二维卷积和三维卷积

**二维卷积**

一般的灰度和彩色图像，只有长宽两个维度，有的是单通道，有的是多通道

**三维卷积**

类似医学扫描图像那种，有长宽高三个维度的立体图像，此时卷积核也是三维的，在输入数据上滑动来提取特征

### 1 * 1 的卷积作用

（1）能够改变输入特征图的通道数，进行降维或者升维的作用
（2）能够让输入特征不同通道上进行卷积操作，实现通道之间的交互
（3）可以重新组织特征，让特征更适合接下来的处理步骤

### 池化操作的原理和功能

（1）它跟卷积操作很类似，也是在一个区域滑动操作处理，只不过它可能取这个区域的最大值，这个是最大池化操作；也可能是取这个区域的平均值，这个是平均池化操作
（2）它的主要作用是减少特征图的维度，降低模型复杂度，从而防止过拟合
